function [ qList, ind] = Read_Coef(chi, xi,dictionary)

if (nargin==2)
    dictionary='F:\WealthDistributionData\WealthDistributionDatabase\';
end

ind=getIndex(chi,xi, dictionary);
ncol=-1; fileNo=-1;
if (chi<0.1)
    ncol=1274; fileNo=0;
else
...
dat=getQDat(fileNo);
entryNo=mod(ind,ncol)+1
qList=dat(:,entryNo);
end

function ind=getIndex(chi,xi, dict)

if (nargin==2)
    dict='F:\WealthDistributionData\WealthDistributionDatabase\';
end

deltaChi=0.002; deltaXi=0.002;
x=round(chi/deltaChi+1); y=round(xi/deltaXi+1);
if (chi<0.1)

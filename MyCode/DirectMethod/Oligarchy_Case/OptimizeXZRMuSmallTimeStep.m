function [DeltaChi, DeltaZeta, gamma, DeltaMu, P0,...
          DeltaP, Qflow, MT, rhs, rhs2] = OptimizeXZRMuSmallTimeStep(chi0, zeta0,...
          chi1, zeta1, n, n_timeStep, dictionary)
%OPTIMIZEXZRMU 
...
DeltaP0=P1-P0;
DeltaP=DeltaP0/n_timeStep; %The change of P in each time step
...
folder_name=char(strcat('Mat',string(numElements)));
if (exist(folder_name,'dir')==0)
    Create_Matrix_to_Disk_Oligarchy(1, 1, chi0, zeta0, 0, numElements, 2);
end
addpath(folder_name);
MatrixM=char(strcat('MatrixM',string(n),'.mat'));
MatrixR=char(strcat('MatrixR',string(n),'.mat'));
MatrixS=char(strcat('MatrixS',string(n),'.mat'));
load(MatrixM);
load(MatrixR);
load(MatrixS);
...
for timestep=1:n_timeStep
    ...
    Qflow=Qflow-MT(:, 2)*DeltaChi-MT(:,3)*DeltaZeta-MT(:,4)*res(4)+rhs;
    pause;
end
